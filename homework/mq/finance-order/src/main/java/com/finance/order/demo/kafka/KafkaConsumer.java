...
public class KafkaConsumer implements ApplicationRunner {
    ...
    public KafkaConsumer(KafkaConfig config) {
        consumer = KafkaUtil.createConsumer(config.getAddress(), Arrays.asList(config.getTopic().split(",")));
        this.config = config;
    }

    @Override
    public void run(ApplicationArguments args) {
        while (config.isCircle()) {
            KafkaUtil.readMessage(consumer, config.getTimeout());
        }
    }
}
